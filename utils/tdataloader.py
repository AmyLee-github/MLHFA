...
def patch_img(img, img_f, patch_size, height):
    ...
    n = 2
    for i in range(n ** 2):
        globals()[f'new_img{i+1}'], globals()[f'new_img_f{i+1}'] = patch_list[i][0], patch_list[i][1]

    new_img = Image.new('RGB', (patch_size * n, patch_size * n))
    new_img_f = Image.new('RGB', (patch_size * n, patch_size * n))

    for i in range(n):
        for j in range(n):
            new_img.paste(globals()[f'new_img{i * n + j + 1}'], (j * patch_size, i * patch_size))
            new_img_f.paste(globals()[f'new_img_f{i * n + j + 1}'], (j * patch_size, i * patch_size))
...
def processing(img, img_f, opt):
    ...
    if opt.isPatch:
        ...
        img_aug_p = transforms.Resize((opt.trainsize, opt.trainsize))(img_aug)
        img_f_p = transforms.Resize((opt.trainsize, opt.trainsize))(img_f)
